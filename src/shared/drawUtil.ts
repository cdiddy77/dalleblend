...
function getTransformationMatrix(src: Keypoint[], dst: Keypoint[]) {
  // Compute the affine transformation matrix that maps the vertices of src to dst
  // const x = src[1].x - src[0].x;
  // const y = src[1].y - src[0].y;
  // const u = src[2].x - src[0].x;
  // const v = src[2].y - src[0].y;

  // const x_ = dst[1].x - dst[0].x;
  // const y_ = dst[1].y - dst[0].y;
  // const u_ = dst[2].x - dst[0].x;
  // const v_ = dst[2].y - dst[0].y;

  // const det = x * v - y * u;
  // if (det === 0) {
  //   throw new Error("Triangles are degenerate");
  // }

  // const a = (x_ * v - y_ * u) / det;
  // const b = (x_ * v_ - y_ * u_) / det;
  // const c = (-x * y_ + y * x_) / det;
  // const d = (-x * v_ + y * u_) / det;
  ...
  // // Compute the transformation matrix elements
  // const a =
  //   (A2.x * B.y -
  //     A2.x * C.y -
  //     B2.x * A.y +
  ...
  //     C2.x * A.x +
  //     C2.x * B.x) /
  //   (A.y * B.x - A.y * C.x - B.y * A.x + B.y * C.x + C.y * A.x - C.y * B.x);
  // const d =
  //   (A2.y * B.x -
  //     A2.y * C.x -
  //     B2.y * A.x +
  ...
  //     C2.y * B.x) /
  //   (A.y * B.x - A.y * C.x - B.y * A.x + B.y * C.x + C.y * A.x - C.y * B.x);
  // const e = A2.x - a * A.x - c * A.y;
  // const f = A2.y - b * A.x - d * A.y;

  // const transformationMatrix = new DOMMatrix([a, b, c, d, e, f]);

  // return transformationMatrix;

  function calculateResetMatrix(p1: Keypoint, p2: Keypoint, p3: Keypoint) {
  ...
  return calculateResetMatrix(src[0], src[1], src[2])
    .invertSelf()
    .preMultiplySelf(calculateResetMatrix(dst[0], dst[1], dst[2]));
}
let countOfPixels = 0;
function transferPixel(
  imageData: ImageData,
  srcX: number,
  srcY: number,
  ctx: CanvasRenderingContext2D,
  ...
  // Check if the (x, y) position is within the bounds of the ImageData object
  if (x >= 0 && y >= 0 && x < imageData.width && y < imageData.height) {
    ...
    countOfPixels++;
    if (countOfPixels % 10000 === 0)
      console.log("pixels placed", countOfPixels);
    ctx.putImageData(pixelData, dstX, dstY);
  } else {
    throw new Error(
      `Position (${x}, ${y}) is out of bounds. ImageData size is (${imageData.width}, ${imageData.height})`
    );
  }
}
// function getPixel(imageData: ImageData, xr: number, yr: number) {
//   // we need round numbers
//   const x = Math.round(xr);
...
// ) {
//   // Check if the (x, y) position is within the bounds of the canvas
//   // Create a new ImageData object with a width and height of 1

//   // Set the pixel RGBA values
//   pixelData.data[0] = rgba.r;
//   pixelData.data[1] = rgba.g;
//   pixelData.data[2] = rgba.b;
//   pixelData.data[3] = rgba.a;

//   // Draw the pixel onto the canvas
//   ctx.putImageData(pixelData, x, y);
// }

// function isPointInTriangle(point: Keypoint, triangle: Keypoint[]) {
...
// function sign(p1: Keypoint, p2: Keypoint, p3: Keypoint) {
//   return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
// }

function iterateTrianglePixels(
  triangle: Keypoint[],
  callback: (x: number, y: number) => void
) {
  // Helper function to sort vertices by their y-coordinates
  function sortVerticesByY(vertices: Keypoint[]) {
    return vertices.slice().sort((a, b) => a.y - b.y);
  }
  // Helper function to interpolate x-coordinates along the triangle edges
  function interpolateX(y: number, v0: Keypoint, v1: Keypoint) {
    return v0.x + ((y - v0.y) * (v1.x - v0.x)) / (v1.y - v0.y);
  }
  // Sort the vertices by their y-coordinates
  const sortedVertices = sortVerticesByY(triangle);
  const [A, B, C] = sortedVertices;
  // Iterate through the rows of the triangle
  for (let y = A.y; y <= C.y; y++) {
    let x1: number;
    let x2: number;
    if (y < B.y) {
      // Upper part of the triangle (A-B-C)
      x1 = Math.ceil(interpolateX(y, A, B));
      x2 = Math.ceil(interpolateX(y, A, C));
    } else {
      // Lower part of the triangle (A-C-B)
      x1 = Math.ceil(interpolateX(y, A, C));
      x2 = Math.ceil(interpolateX(y, B, C));
    }
    if (x1 > x2) {
      [x1, x2] = [x2, x1];
    }
...
export function drawResults(
  ...
  faces.forEach((face) => {
    const keypoints = face.keypoints.map(
      (keypoint): point => [keypoint.x, keypoint.y]
    );
...
export function createStretchedImage(
  imgData: ImageData | null,
  face: Meshpoint[] | undefined,
  stretchFace: Meshpoint[]
): HTMLCanvasElement | null {
  countOfPixels = 0;
  const start = performance.now();
  console.log("starting createStretchedImage");
  if (imgData == null || !face) return null;
  const outputCanvas = document.createElement("canvas");
  // outputCanvas.width = imgData.width;
  // outputCanvas.height = imgData.height;
  outputCanvas.width = 1024;
  outputCanvas.height = 1024;
  const outputCtx = outputCanvas.getContext("2d");
  if (outputCtx == null) return null;
  const pixelData = outputCtx.createImageData(1, 1);
  console.log("starting loop", TRIANGULATION.length / 3);
  // for each pair of triangles in the face
  for (let i = 0; i < TRIANGULATION.length / 3; i++) {
    console.log("loop", i);
    // if (i > 50) break;
    const srcPoints = [
    ...
    const dstPoints = [
      ...
      TRIANGULATION[i * 3 + 1],
      TRIANGULATION[i * 3 + 2],
    ].map((index) => stretchFace[index]);
    // get the transformation matrix from the source to the destination
    const matrix = getTransformationMatrix(dstPoints, srcPoints);
    try {
      // console.log(srcPoints, dstPoints, matrix);
      // iterate over every pixel in the destination triangle
      iterateTrianglePixels(dstPoints, (x, y) => {
        // get the corresponding pixel in the source triangle
        const srcPoint = matrix.transformPoint({ x, y });
        // if (!isPointInTriangle(srcPoint, srcPoints)) {
        //   console.log("not in triangle", srcPoint, srcPoints);
        // }
        // draw the source pixel to the destination pixel
        const px = Math.min(imgData.width - 1, Math.max(0, srcPoint.x));
        const py = Math.min(imgData.height - 1, Math.max(0, srcPoint.y));
        transferPixel(imgData, px, py, outputCtx, pixelData, x, y);
        // const pixel = getPixel(imgData, px, py);
        // setPixel(outputCtx, pixelData, x, y, pixel);
        // console.log(srcPoint, x, y, pixel);
      });
    } catch (e) {
      const err = e as Error;
      console.error(err.message, i, srcPoints, dstPoints);
    }
  }
  // finally, draw the image to the canvas
  console.log(
    "done createStretchedImage",
